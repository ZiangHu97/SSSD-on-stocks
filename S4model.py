...
def cauchy_slow(v, z, w):
            ...
            returns: (..., L)
            """
            cauchy_matrix = v.expand_dims(-1) / (z.expand_dims(-2) - w.expand_dims(-1)) # (... N L)
            return tf.sum(cauchy_matrix, dim=-2)

def _broadcast_dims(*tensors):
    max_dim = max([len(tensor.shape) for tensor in tensors])
    tensors = [tensor.reshape((1,)*(max_dim-len(tensor.shape))+tensor.shape) for tensor in tensors]
    return tensors

_c2r = tf.complex
_r2c = tf.math.real
_conj = lambda x: tf.concat([x, tf.math.conj(x)], dim=-1)

_resolve_conj = lambda x: tf.conj(x)
...
def krylov(L, A, b, c=None, return_power=False):
    ...
    while not done:
        ...
        _x = A_ @ _x
        x = tf.concat([x, _x], dim=-1) # there might be a more efficient way of ordering axes
...
class TransposedLinear(keras.layers.Layer):
    ...
    def __init__(self, d_input, d_output, bias=True, initializer=None):
        super().__init__()
        if initializer is None:
            initializer = tf.keras.initializers.HeUniform()
        
        self.weight = initializer(shape=(d_output, d_input))
        # self.weight = tf.Variable(tf.empty([d_output, d_input]))
...
def LinearActivation(
    ...
    linear_cls = TransposedLinear if transposed else keras.Layer.linear
    if activation == 'glu': d_output *= 2
    linear = linear_cls(d_input, d_output, bias=bias,initializer=initializer, **kwargs)
    ...
    if bias and zero_bias_init:
        linear.bias = keras.initializers.Zeros()(shape=linear.bias.shape)
...
def rank_correction(measure, N, rank=1, dtype=tf.float):
    """ Return low-rank matrix L such that A + L is normal """

    if measure == 'legs':
        assert rank >= 1
        P = tf.sqrt(.5+tf.range(N, dtype=dtype)).expand_dims(0) # (1 N)
    elif measure == 'legt':
        assert rank >= 2
...
def randint( low , high=None, size= None,dtype=None):
    ...
    ## The equvilant code to the torch.randn()
    return tf.convert_to_tensor(np.random.randint(low,high=high ,size=size),dtype=dtype)
...
def nplr(measure, N, rank=1, dtype=tf.float32):
    ...
    if measure == 'random':
        ...
        w = -tf.exp(randint(N//2)) + 1j*randint(N//2)
        P = randint(rank, N//2, dtype=dtype)
        B = randint(N//2, dtype=dtype)
    ...
    AP = A + tf.sum(tf.expand_dims(P,-2)*tf.expand_dims(P,-1), dim=-3)
...
class SSKernelNPLR(keras.layers.Layer):
    ...
    def __init__(
        ...
        C = C.expand_dims(tf.broadcast_to(C.shape, (1, self.H, self.N))) # (H, C, N)
        ...
        if self.L is not None:
            self._omega(self.L, dtype=C.dtype)
    ...
    def call(self, state=None, rate=1.0, L=None):
        ...
        B = tf.concat([B, P], dim=-3) # (s+1+r, H, N)
        C = tf.concat([C, Q], dim=-3) # (c+r, H, N)

        # Incorporate B and C batch dimensions
        # v = B.unsqueeze(-3) * C.unsqueeze(-4)  # (s+1+r, c+r, H, N)
        v=tf.expand_dims(B,-3)+tf.expand_dims(C,-4)
    ...
    def setup_step(self, mode='dense'):
        ...
        dA_L = power(self.L, self.dA)
        I = tf.cast(tf.eye(self.dA.shape[-1]),dtype=dA_L.dtype)
        C = _conj(_r2c(self.C)) # (H C N)

        perm=range(len(dA_L.shape()))
        perm[-1],perm[-2] = perm[-2], perm[-1]
        dC = tf.squeeze(tf.linalg.solve(
            I - tf.transpose(dA_L,perm),
            tf.expand_dims(C,-1),
        ),-1)
        self.dC = dC
    ...
    def register(self, name, tensor, trainable=False, lr=None, wd=None):
        ...
        setattr(self,name,tf.Variable(tensor,trainable=trainable))
...
class HippoSSKernel(keras.layers.Layer):
    ...
    def __init__(
        ...
        dtype = tf.float64 if self.precision == 2 else tf.float32
        ...
        C = tf.random.uniform([channels, self.H, self.N // 2], dtype=cdtype)
...
def get_torch_trans(heads=8, layers=1, channels=64):
    ...
    return tfm.nlp.models.TransformerEncoder(num_layers=layers,num_attention_heads=heads,intermediate_size=channels,activation='gelu',)
...
class S4(keras.layers.Layer):
    ...
    def __init__(
        ...
        super().__init__()
        if verbose:
            # import utility.train
            # log = utility.train.get_logger(__name__)
        ...
        self.D = tf.Variable(randint([channels, self.h]))
...
class S4Layer(keras.layers.Layer):
    ...
    def __init__(self, features, lmax, N=64, dropout=0.0, bidirectional=True, layer_norm=True):
        ...
        self.norm_layer = keras.layers.LayerNormalization(axis=-1) if layer_norm else keras.layers.Identity()
