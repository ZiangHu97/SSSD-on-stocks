...
def calc_diffusion_hyperparams(T, beta_0, beta_T):
    ...
    for t in range(1, T):
        ...
        # / (1-\bar{\alpha}_t)
    Sigma = tf.sqrt(Beta_tilde)  # \sigma_t^2  = \tilde{\beta}_t
...
def sampling(net, size, diffusion_hyperparams, cond, mask, only_generate_missing=0, guidance_weight=0):
    ...
    Parameters:
    net (tf network):            the wavenet model
    ...
    with tape.stop_recording():
        for t in range(T - 1, -1, -1):
            if only_generate_missing == 1:
                x = x * tf.cast(1 - mask,float)+ cond * tf.cast(mask,float)
...
def training_loss(net, loss_fn, X, diffusion_hyperparams, only_generate_missing=1):
    ...
    B, C, L = tf.shape(audio)  # B is batchsize, C=1, L is audio length
    diffusion_steps = tf.convert_to_tensor(np.random.randint(T, size=(B, 1, 1)))  # randomly sample diffusion steps from 1~T

    z = std_normal(tf.shape(audio))
    if only_generate_missing == 1:
        z = audio * tf.cast(mask, float) + z * tf.cast(1 - mask,float)
    transformed_X = tf.sqrt(Alpha_bar[diffusion_steps]) * audio + tf.sqrt(
        1 - Alpha_bar[diffusion_steps]) * z  # compute x_t from q(x_t|x_0)
    epsilon_theta = net(
        (transformed_X, cond, mask, tf.reshape(diffusion_steps,[B, 1])))  # predict \epsilon according to \epsilon_\theta
...
def get_mask_rm(sample, k):
    ...
    length_index = tf.convert_to_tensor(range(tf.shape(mask)[0]))  # lenght of series indexes
    for channel in range(tf.shape(mask)[1]):
        perm = tf.random.shuffle(len(length_index))
        idx = perm[0:k]
        mask[:, channel][idx] = 0
...
def get_mask_mnr(sample, k):
    ...
    mask = tf.ones(tf.shape(sample))
    length_index = tf.convert_to_tensor(range(tf.shape(mask)[0]))
